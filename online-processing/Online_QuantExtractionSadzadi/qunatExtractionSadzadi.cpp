...
// bit streams using ---- (Sadzadi et al. qunatizer - Reference - "Robust key generation
...
void getAvgSD_Bits(int8_t x[], int8_t n, float* avg, float* sd, uint8_t* len, float lFactor, float rFactor) {
    ...
    *sd = (float) sqrt(var);
    ...
    //calculate Left thresholds (As per Algorithm). Single Threashold, hence Rth = Lth
   leftThresh = ( *avg - lFactor*(*sd) );
   rightThresh = leftThresh;
...
Serial.print("***SADZADI***  Avg : "); Serial.print(*avg); Serial.print("  Var : "); Serial.print((float) var); Serial.print("  Std Dev : "); Serial.print(*sd);  Serial.print(" Thresholds -> LT : "); Serial.print(leftThresh); Serial.print(" RT : "); Serial.println(rightThresh);
    ...
    *len = j;
    ...
    Serial.print("Extrac Bits(Sadzadi) (");
    Serial.print(*len);
    Serial.print(") : ");
...
int processArrayData(){
	...
	float alpha_left = 3;  // Large multiple of Standard Deviation for Deep Thresholds
	float alpha_right = 3; //single Threshold, alpha_right is not used for Th calculation in Sadzadi
