...
plotFunc = function(x_A, x_B, mA, mB,stdDevA, stdDevB, rngA1, rngA2, rngB1, rngB2, algo, dirPath, rowcount, modFlag){
  ...
  ####   PLOTTING COMMENCES ##########################  
  plotfileA <- paste0(dirPath,rowcount,"_NodeE_B_", algo)
  png(plotfileA)
  ...
  plot(x_A$x, x_A$freq, type='l', col="black", lty="solid", lwd=2, xlim = c(min(x_A$x)-1,max(x_A$x)+1), ylim = c(0,max(x_A$freq)+150), 
       ...
       main = paste0("Frequency Plot of NodeE_B RSSI values (", algo ," et al.)")) 
...
#  inFileB="nodeB.txt"
  ...
  workDir<-getwd()
  if (modFlag==0)
  dirPath<-paste0(workDir,"/SNIFFER_GRAPHS/EVE/",algo, "/")
  else if (modFlag==1)
    dirPath<-paste0(workDir,"/SNIFFER_GRAPHS/EVE/",algo, "/Modified/")
...
# graphPlot("nodeB_B.txt", "nodeB_A.txt", "Sadzadi", 1)
# 
