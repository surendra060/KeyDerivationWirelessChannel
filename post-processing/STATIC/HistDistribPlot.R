...
plotFunc = function(x_A, x_B, mA, mB,stdDevA, stdDevB, rngA1, rngA2, rngB1, rngB2, algo, dirPath, rowcount, modFlag){
  ...
  ####   PLOTTING COMMENCES ##########################  
  plotfileA <- paste0(dirPath,rowcount,"_NodeA_", algo)
  png(plotfileA)

  hgrid = seq(0, max(x_A$freq)+100, by=50)
  
  plot(x_A$x, x_A$freq, type='l', col="black", lty="solid", lwd=2, xlim = c(min(x_A$x)-1,max(x_A$x)+1), ylim = c(0,max(x_A$freq)+50), 
       xlab = "Received Signal Strength Index (RSSI)", ylab = "Frequency", 
       main = paste0("Frequency Plot of Node-A RSSI values (", algo ," et al.)")) 
  ...
  if ((algo == "Mathur") | (algo == "Sadzadi")){
    ...
    legend("topleft", c("Frequency distribution","Mean RSSI value", "Approx. Normal distribution","Threshold RSSI") , 
    ...
    legend("topleft", c("Frequency distribution","Median RSSI value","Threshold RSSI") , 
  ...
  plotfileB <- paste0(dirPath,rowcount,"_NodeB_", algo)
  png(plotfileB)
  
  plot(x_B$x, x_B$freq, type='l', col="black", lty="solid", lwd=2, xlim = c(min(x_B$x)-1,max(x_B$x)+1), ylim = c(0,max(x_B$freq)+50), 
       xlab = "Received Signal Strength Index (RSSI)", ylab = "Frequency", 
       main = paste0("Frequency Plot of Node-B RSSI values (", algo ," et al.)")) 
...
#  inFileB="nodeB.txt"
  ...
  workDir<-getwd()
  if (modFlag==0)
  dirPath<-paste0(workDir,"/STATIC_GRAPHS/",algo, "/")
  else if (modFlag==1)
    dirPath<-paste0(workDir,"/STATIC_GRAPHS/",algo, "/Modified/")
  
  #filename<-"DensityPlot.png"
  
  for (rowcount in 150:160) {
...
graphPlot("nodeA.txt", "nodeB.txt", "Aono", 0)
graphPlot("nodeA.txt", "nodeB.txt", "Aono", 1)

graphPlot("nodeA.txt", "nodeB.txt", "Tope", 0)
graphPlot("nodeA.txt", "nodeB.txt", "Tope", 1)

graphPlot("nodeA.txt", "nodeB.txt", "Sadzadi", 0)
graphPlot("nodeA.txt", "nodeB.txt", "Sadzadi", 1)

graphPlot("nodeA.txt", "nodeB.txt", "Mathur", 0)
graphPlot("nodeA.txt", "nodeB.txt", "Mathur", 1)
