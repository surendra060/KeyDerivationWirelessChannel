...
nodeData_Tope = function(nodeRSSA, infileA, outfileBitStringA,outfileDataA, nodeRSSB, infileB, outfileBitStringB,outfileDataB, entropyTestFile){
  ...
  for(rowcount in rowindex)
  {
    ...
    meanValueA = mean(nodeRSSA[rowcount, colindex])
    stdDevA = sd(nodeRSSA[rowcount, colindex])
    meanValueB = mean(nodeRSSB[rowcount, colindex])
    stdDevB = sd(nodeRSSB[rowcount, colindex])
    ...
    LeftThA = round(meanValueA) - alpha
    RightThA = round(meanValueA) + alpha
    LeftThB = round(meanValueB) - alpha
    RightThB = round(meanValueB) + alpha
...
nodeData_Tope(A, "nodeA.txt", "nodeA_BS_MODTope.txt", "nodeA_BS_data_MODTope.txt", B, "nodeB.txt", "nodeB_BS_MODTope.txt", "nodeB_BS_data_MODTope.txt", "nodeAB_MODTope_Entropy.txt")
print(paste("TX-#0's : TX - #1's "))
print(paste(system("grep -o '0' nodeA_BS_MODTope.txt | wc -l", wait=TRUE), "<->", system("grep -o '1' nodeA_BS_MODTope.txt | wc -l", wait=TRUE)))
